...
class MonitorRecordConfig(RecordConfig):
    observation_poll_hz: float = 90.0
    ...
    def __post_init__(self):
        ...
        if self.policy is None or self.policy.type != "remote_monitor":
            raise ValueError("`lerobot-human-inloop-record-monitor` requires `--policy.type=remote_monitor`.")
...
@parser.wrap()
def human_inloop_record_monitor(cfg: MonitorRecordConfig):
    if cfg.teleop is None:
        raise ValueError("`lerobot-human-inloop-record-monitor` requires `teleop` config.")
    ...
    init_logging()
    logging.info(
        "Dual monitor human-in-loop recording is enabled. ACP inference: enable=%s use_cfg=%s cfg_beta=%.3f. "
    ...
    if cfg.display_data:
        init_rerun(session_name="recording_monitor", ip=cfg.display_ip, port=cfg.display_port)
    ...
    try:
        ...
        if teleop is None or isinstance(teleop, list):
            raise ValueError(
                "`lerobot-human-inloop-record-monitor` requires exactly one teleoperator with send_feedback support."
            )
        ...
        with dataset_context:
            ...
            while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
                ...
                if callable(getattr(cfg, "_on_record_episode_outcome", None)):
                    cfg._on_record_episode_outcome(robot, teleop, episode_success)
                ...
                if (
                    ...
                    and ((recorded_episodes < cfg.dataset.num_episodes) or events["rerecord_episode"])
                ):
