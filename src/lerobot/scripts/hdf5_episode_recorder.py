...
class HDF5EpisodeRecorder:
    ...
    def _discover_existing_episode_count(self) -> int:
        existing = sorted(self.root.rglob("episode_*.hdf5"))
    ...
    def _resolve_episode_dir(self, episode_success: str | None) -> Path:
        if episode_success == EPISODE_SUCCESS:
            subdir = "success"
        elif episode_success == EPISODE_FAILURE:
            subdir = "fail"
        else:
            subdir = "unlabeled"
        episode_dir = self.root / subdir
        episode_dir.mkdir(parents=True, exist_ok=True)
        return episode_dir

    def _discover_next_episode_index(self, episode_dir: Path) -> int:
        existing = sorted(episode_dir.glob("episode_*.hdf5"))
        episode_indices: list[int] = []
        for path in existing:
            stem = path.stem
            try:
    ...
    def save_episode(self, extra_episode_metadata: dict[str, Any] | None = None) -> Path | None:
        ...
        episode_success = None if not extra_episode_metadata else extra_episode_metadata.get("episode_success")
        episode_dir = self._resolve_episode_dir(episode_success)
        episode_idx = self._discover_next_episode_index(episode_dir)
        file_path = episode_dir / f"episode_{episode_idx}.hdf5"
