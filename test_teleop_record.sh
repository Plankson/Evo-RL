...
if [ "${TESTMODE}" = "true" ]; then
  DATASET_BASE_DIR="${TMPDIR:-/tmp}/evorl_dataset_testmode/${COLLECTION_NAME}/${DAY_FOLDER}"
  DATASET_ROOT="${DATASET_BASE_DIR}/${DATASET_NAME}"
  rm -rf "${DATASET_ROOT}"
  trap 'rm -rf "${DATASET_ROOT}"' EXIT
else
  DATASET_BASE_DIR="${HOME}/evorl_dataset/${COLLECTION_NAME}/${DAY_FOLDER}"
  DATASET_ROOT="${DATASET_BASE_DIR}/${DATASET_NAME}"
fi

DATASET_REPO_ID="ACE_ROBOTICS/${COLLECTION_NAME}_${DAY_FOLDER}_${DATASET_NAME}"
mkdir -p "${DATASET_BASE_DIR}"

echo "Saving dataset to: ${DATASET_ROOT}"
echo "Teleop-only record: leader arms control follower arms, no policy."
if [ -n "${TAG}" ]; then
