...
# PROMPT="PUT THE PEN INTO THE PEN HOLDER"
 # left serial number： 346522074444
 # right serial number： 346522074314
POLICY_NAME="pi0"
# POLICY_NAME="ace_policy"
PORT=8886
# PORT=8080
TAG="stack all plates"
TESTMODE="false"
...
if [ "${TESTMODE}" = "true" ]; then
  DATASET_BASE_DIR="${TMPDIR:-/tmp}/evorl_dataset_testmode/${POLICY_NAME}"
  DATASET_ROOT="${DATASET_BASE_DIR}/${TAG_SLUG}_${RUN_TIMESTAMP}"
  rm -rf "${DATASET_ROOT}"
  trap 'rm -rf "${DATASET_ROOT}"' EXIT
else
  DATASET_BASE_DIR="${HOME}/evorl_dataset/${POLICY_NAME}"
  DATASET_ROOT="${DATASET_BASE_DIR}/${TAG_SLUG}_${RUN_TIMESTAMP}"
fi
...
args=(
  ...
  --dataset.record_camera_keys='["observation.images.right_front"]'
  --dataset.save_policy_frame_ids=true
